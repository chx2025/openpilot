...
class LaneCenteringCorrector:
  def __init__(self) -> None:
    ...
    self._filter = FirstOrderFilter(0.0, _SMOOTH_TAU, 0.01)
  ...
  def _log_fallback(self, state: str, v_ego_kph, lat_active):
    self._log_row(state, dt=0, v_kph=v_ego_kph, lat_active=lat_active,
                  speed_gate=self._speed_gate, lane_width=0, target_y=0,
                  model_y=0, error=0,
                  raw_correction=0, correction=self._correction)
  ...
  def update(self, model_v2, v_ego: float, lat_active: bool, dt: float,
    ...
    if not self._enabled or not lat_active or not self._speed_gate or v_ego < _MIN_V_EGO:
      self.reset()
      self._log_fallback("DISABLED_OR_SLOW", v_ego_kph, lat_active)
      return 0.0
    ...
    if self._yield_hold_timer >= YIELD_CONFIRM_SEC:
      self._filter.rc = _SIGNAL_RELEASE_TAU
      self._correction = self._filter.update(0.0)
      self._log_fallback("YIELDING_OR_SIGNAL", v_ego_kph, lat_active)
      return self._correction

    try:
      if model_v2.meta.laneChangeState != log.LaneChangeState.off:
        self.reset()
        self._log_fallback("LANE_CHANGE", v_ego_kph, lat_active)
        return 0.0
    except (AttributeError, TypeError, ValueError):
      self.reset()
      return 0.0

    # 執行 Starpilot 核心誤差計算 (已移除 E2E 退讓)
    valid, raw_correction, log_data = self._calculate_raw_correction(model_v2, v_ego)
    
    if not valid:
      # 當信心不足或無法計算時，平滑淡出
      self._filter.rc = _CONFIDENCE_RELEASE_TAU
      self._correction = self._filter.update(0.0)
      self._log_row("INVALID_LANE", v_kph=v_ego_kph, lat_active=lat_active, speed_gate=self._speed_gate, **log_data)
      return self._correction

    # 計算最終修正量並進行平滑處理 (回復正常平滑係數)
    self._filter.rc = _SMOOTH_TAU
    target = float(np.clip(raw_correction, -_MAX_RAW_CORRECTION, _MAX_RAW_CORRECTION)) * _MAX_GAIN
    self._correction = self._filter.update(target)

    self._log_row("ACTIVE", v_kph=v_ego_kph, lat_active=lat_active, speed_gate=self._speed_gate, 
                  raw_correction=raw_correction, correction=self._correction, **log_data)
  ...
  def _calculate_raw_correction(self, model_v2, v_ego: float):
    ...
    try:
      ...
      # 嚴格過濾：信心度太低或標準差太高，視為無效車道線 (數值已放寬)
      ...
      # 已完全移除 E2E Authority (break_in) 機制
