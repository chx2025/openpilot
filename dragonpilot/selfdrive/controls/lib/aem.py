...
class Config:
    ...
    EXIT_DEBOUNCE_FRAMES = 15         # 需要連續低於閾值15幀才完全退出 (修正：防暴衝，延長確認時間)
...
class ModeTransitionManager:
  ...
  def request_mode(self, mode, confidence=1.0):
    ...
    # (修正：調高 Blended 模式的增加步長，解開無法切換的數學死結)
    step = 0.2 if (mode == Config.MODE_ACC and confidence >= 0.9) else 0.15
  ...
  def update(self, urgency_val):
    ...
    # 自然衰減 Blended 模式信心
    # (修正：放寬衰減率，讓信心值不會被扣得太快而永遠達不到門檻)
    self.mode_confidence[Config.MODE_BLENDED] *= 0.98
...
class AEM:
    ...
    def update_states(self, model_msg, radar_msg, v_ego):
        """主邏輯更新"""
        # 資料完整性檢查 (修正：將 position.z 改為 position.x，讀取正確的縱向距離)
        if len(model_msg.position.x) != ModelConstants.IDX_N:
            return

        v_kph = v_ego * 3.6
        # 直接取最後一點的距離 (修正：將 position.z 改為 position.x)
