...
| P3 | Local Defaults/UX | complete Simplified Chinese catalog, legacy C3XL GPIO42 buzzer/sounds, offroad and on-road brightness controls, one-minute shutdown choice, speed offset cap, eGPU status/telemetry and safe eject | separate defaults policy and isolated UI rows | Translation/catalog and device process checks pass; audible/display checks pending |
...
not require copying the planner. Traffic control is a Plan Constraint with
