...
from openpilot.selfdrive.debug.device_console_auth import authorize, client_is_local, require_offroad
...
<head>
  ...
  <h1>车载设置</h1><p>通过手机或电脑访问此页面。所有 API 都需要设备屏幕显示的随机访问令牌。</p>
  <div class="terminal-row"><input id="device-token" type="password" autocomplete="off" placeholder="设备控制台访问令牌"><button onclick="saveDeviceToken()">使用令牌</button></div>
...
deviceTokenInput.value = sessionStorage.getItem('openpilotDeviceConsoleToken') || '';
function saveDeviceToken() { sessionStorage.setItem('openpilotDeviceConsoleToken', deviceTokenInput.value); loadSettings(); }
function apiFetch(url, options = {}) { options.headers = {...(options.headers || {}), 'X-Device-Token':deviceTokenInput.value}; return fetch(url, options); }
...
class TurnSignalHandler(BaseHTTPRequestHandler):
  ...
  def _authorize_api(self) -> bool:
    try:
      ...
      authorize(self.headers.get("X-Device-Token"))
      return True
  ...
  def do_POST(self) -> None:
    ...
    if self.path == "/api/terminal/exec":
      try:
        ...
        self._json(HTTPStatus.OK, run_command(payload["command"], self.headers.get("X-Device-Token")))
      except PermissionError as error:
        self._json(HTTPStatus.FORBIDDEN, {"ok": False, "message": str(error)})
      except (TypeError, ValueError, json.JSONDecodeError) as error:
        self._json(HTTPStatus.BAD_REQUEST, {"ok": False, "message": str(error)})
