...
`selfdrive/carrot/carrot_functions.py`, verified line-by-line against carrot's
real source, and further corrected against several real-world road-test
reports. Every "FIXED" comment below marks a place where an earlier port pass
got this wrong and the fix is now locked in by a unit test.

NO UI IN THIS VARIANT: by request, this fork does not get a settings toggle.
Edit TRAFFIC_STOP_ENABLED / TRAFFIC_STOP_DISTANCE_ADJUST_M below directly and
rebuild -- there is no Params dependency in this file at all.
"""
...
# ============================================================================
# IN-CODE CONFIGURATION -- edit directly, no UI toggle in this fork.
# ============================================================================
TRAFFIC_STOP_ENABLED = True

# User-facing fine-tune, layered on top of the fixed TRAFFIC_STOP_CAMERA_TO_FRONT_M
# baseline below (see that constant's comment for why the two are kept separate).
...
# of ~2m (see conversation history: at the boundary the MPC ends up targeting the unbuffered
# virtual stop-line obstacle instead of the real lead's own, more comfortable following distance).
LEAD_CLOSE_TO_STOP_LINE_M = 4.0
...
# cp's single `TrafficStopDistanceAdjust` conflates two different things into one number: a
# large, roughly-fixed physical correction for the model's stop-line distance being measured
# from the camera/device position (set back from the actual front bumper) rather than from the
# bumper itself, AND a small personal fine-tune on top of that. cp's real shipped default is raw
# Params value -150 (see params_keys.h in forks that have a UI for this), divided by 100 ->
# -1.5m. Here the two are split: this constant is applied unconditionally as a fixed baseline,
# and TRAFFIC_STOP_DISTANCE_ADJUST_M above is a genuine small fine-tune on top of it.
...
# universal constant -- it has not been independently re-derived or verified against this fork's
# own calibration pipeline or hardware mounting position.
...
class TrafficStopController:
  def __init__(self):
    self._enabled = TRAFFIC_STOP_ENABLED
    self._distance_adjust_m = TRAFFIC_STOP_DISTANCE_ADJUST_M
    ...
    # cp's vFilter/xStopFilter/xStopFilter2 run every frame unconditionally and are never
    # cleared -- they are NOT reset on release, so these histories persist across separate stop
    # events (only the `_stop_x_rl` rate-limiter output, and the accumulator below, are reset).
    # Keeping the raw-model filters continuously warm means a fresh red-light entry starts from
    # an already-settled number instead of a median/average of just 1-2 samples.
  ...
  def _reset(self):
    ...
    _model_v_hist / _stop_x_median_hist / _stop_x_avg_hist -- see note in __init__."""
    self._state = TrafficStopState.CRUISE
    self._stopped_grace_frames = 0
    self._stop_sign_count = 0
    self._start_sign_count = 0
  ...
  def update(self, model_v2, car_state, radar_state, v_ego: float, a_ego: float, v_cruise: float) -> TrafficStopResult:
    ...
    # 10s post-gas-press suppression window: only arms while actively braking toward a stop
    # (STOPPING), not on every gas press during ordinary driving.
    ...
    # close-lead check for CANCELLING an active stop compares against stop_model_x_raw (filtered,
    # not rate-limited) with a margin. The gate for entering a NEW stop is different (below).
    ...
    if self._state == TrafficStopState.CRUISE:
      # Entry is blocked by *any* detected lead at all (cp: `if lead_detected: xState =
    ...
    # cause -- see porting notes. Crucially this must NOT also force `_state` back to CRUISE: in
    # cp, this override only zeroes the local distance math for this frame -- self.xState is
    # untouched by it. The state machine block above is the only thing allowed to change `_state`.
    if self._state == TrafficStopState.CRUISE:
      self._actual_stop_distance = 0.0
      self._reference_speed_kph = 0.0
      self._stopped_grace_frames = 0
    ...
    # while actively stopping, resync the rate-limiter's internal state to the raw filtered value
    # every frame, which hands authority for smoothing over to _actual_stop_distance's
    ...
    # the fixed camera-to-front baseline is always applied, in addition to (not instead of) the
    # small fine-tune -- see the TRAFFIC_STOP_CAMERA_TO_FRONT_M comment above. Applied exactly
    # once, here.
    stop_dist = get_traffic_stop_obstacle_distance(stop_dist, TRAFFIC_STOP_CAMERA_TO_FRONT_M + self._distance_adjust_m)

    # STOPPED hard-forces v_cruise to 0 rather than relying on the soft sqrt formula the whole
    # time, so full stop doesn't creep due to model noise in stop_dist.
