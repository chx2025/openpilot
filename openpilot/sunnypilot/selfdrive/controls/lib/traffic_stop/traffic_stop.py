...
`selfdrive/carrot/traffic_stop.py`, cross-verified against carrot's own
`tests/test_traffic_stop.py` pytest suite (exact numeric matches).
...
def get_traffic_stop_obstacle_distance(stop_distance: float, distance_adjust: float) -> float:
  """Apply a fixed/adjustable offset to the stop distance, clamped to 0."""
