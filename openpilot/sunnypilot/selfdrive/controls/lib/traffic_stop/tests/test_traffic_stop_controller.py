...
class MockSmallModelPlan:
  """Mirrors cereal ModelDataV2SP.smallModelPlan (capnp struct -> duck-typed mock)."""
  def __init__(self, valid=False, positionX=None, positionY=None, velocityX=None):
    self.valid = valid
    self.positionX = positionX if positionX is not None else [0.0] * 33
    self.positionY = positionY if positionY is not None else [0.0] * 33
    self.velocityX = velocityX if velocityX is not None else [0.0] * 33


class TestSmallModelPlanPreference:
  """When the chestnut (eGPU) big model is active, modeld runs the small model in parallel and
  publishes its plan on modelDataV2SP.smallModelPlan. The controller must drive the entire
  red-light state machine from that plan when valid, and transparently fall back to modelV2
  (the active model) when it is not."""

  def _make(self):
    # big model: gentle decel, stop line far -> no red-light detection
    big = approaching_red_light_model(model_x_end=150.0, model_v_start=20.0, model_v_end=18.0)
    # small model: hard decel to a close stop line -> red-light detection
    small = MockSmallModelPlan(valid=True, positionX=[12.0] * 33, positionY=[0.0] * 33,
                               velocityX=[8.0] * 1 + [7.0] + [2.0] * 31)
    return big, small

  def test_valid_small_plan_drives_state_machine(self):
    big, small = self._make()
    controller = TrafficStopController()
    result = run_frames(controller, big, MockCarState(), MockRadarState(),
                        v_ego=8.0, a_ego=0.0, v_cruise=8.0, n=12)
    assert controller._state == TrafficStopState.CRUISE  # big model alone: no stop
    # now with small plan: same big model, stop comes from the small model
    for _ in range(12):
      result = controller.update(big, MockCarState(), MockRadarState(),
                                 8.0, 0.0, 8.0, small_model_plan=small)
    assert controller._state == TrafficStopState.STOPPING
    assert result.stop_dist_m is not None
    assert result.v_cruise_limited is not None

  def test_invalid_small_plan_falls_back_to_active_model(self):
    big, small = self._make()
    small.valid = False
    controller = TrafficStopController()
    for _ in range(12):
  ...
  def test_none_small_plan_is_back_compatible(self):
    controller = TrafficStopController()
    model = approaching_red_light_model()
    result = run_frames(controller, model, MockCarState(), MockRadarState(),
                        v_ego=7.0, a_ego=0.0, v_cruise=7.0, n=1)
    assert controller._state == TrafficStopState.STOPPING  # existing behaviour unchanged

  def test_truncated_small_plan_falls_back(self):
    big, small = self._make()
    small.positionX = small.positionX[:2]
    controller = TrafficStopController()
    for _ in range(12):
      result = controller.update(big, MockCarState(), MockRadarState(),
                                 8.0, 0.0, 8.0, small_model_plan=small)
