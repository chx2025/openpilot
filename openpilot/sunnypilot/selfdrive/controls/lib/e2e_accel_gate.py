...
============================================================================
第 4 次修订（2026-09-24）：**⑤ 不再重复判「已被雷达融合的前车」**
============================================================================
用户纠正（原话）：「不肯加速不是加速度不够，而是明明要加速的，被某种力量给压制了，
而这种压制是踩了油门后。」

量化取证（220 帧 `[E2eTrace]` 回放，`dc8f940` 版实车）：
  · 「该加速却没加速」的欠速帧 **261** 个中，**e2e 候选胜出 236（90%）**、
    mpc 22（8%）、**cruise 表值太小只 3（1%）** ⇒ 压制源是 e2e，**不是表值**。
  · `long_accel_limiter` 只动 **2 帧（1%）** ⇒ 也不是它
    （曾据「每帧恰好 +0.10 = 2.0 m/s³ × 0.05 s」误判为限幅器指纹，已用留痕 `lim`/`aPre` 更正）。
  · `aE2e` 按欠速量分桶：`gap < 8 km/h` 时 **aE2e > +0.3 占 0%**（模型 100% 不给加速）；
    8–18 km/h 才 14~15%，18–25 km/h 36%。
    ⇒ 踩油门把 `gap` 压小，正好推进「绝不给加速」区间；松开后只剩 +0.1 m/s²，
    **连行驶阻力都克服不了 ⇒ 车真的在掉速**（实测 50→42 km/h，掉 8 kph）。
  · 门控为何没救住：①（欠速门槛 7.2 km/h）恰好覆盖模型死区，**①本身没问题**；
    卡住的是 **⑤** —— 它在 **62~104 m 的远车**上反复触发
    （`lead=1 dRel=70~104 prob=0.60~0.90`），而那辆车 ④ 已经按 `dRel ≤ 50 m` 判过一次。

机理：`radarState.leadOne` **已融合视觉单源目标**（留痕原文
`lead=1 dRel=87.4 vLeadK=4.5 prob=0.73 radar=0`，`radar=0` 即视觉单源）
⇒ ④ 与 ⑤ **判的是同一个物理目标**，而 ⑤ 的门槛（概率 0.5）远比 ④ 的门槛（距离 50 m）松
—— 等于用一条更松的判据把 ④ 的结论整个推翻。
**这是判据重复，属逻辑缺陷，不是阈值取舍。**

改法（**只此一处判据，不动任何门槛值**）：
  · 软条件 ⑤：`model_lead_prob >= E2E_GATE_LEAD_PROB` → 追加 `and not lead_present`
  · 硬闸 `hardProb`：同口径追加 `and not lead_present`
    （否则会出现「软条件放行、硬闸立刻关」的自相矛盾）
  · 保持态的滞回门槛（0.6）走同一个 `should_yield()` ⇒ 自动同口径，无需另改。

离线回放（220 帧）：

  | 口径 | 欠速帧 e2e 让位 | 仍「该加速却没加速」 | 欠速帧平均 aTgt |
  |------|----------------|---------------------|-----------------|
  | 改前（现状） | 19/204 (9%) | 134 | −0.005 |
  | **本版（只改 ⑤）** | **75/204 (37%)** | **101（救回 33）** | **+0.100** |

安全性（相对改前的变化面）：
  1. **未融合**的高置信前车（`lead_present=False`，即「雷达没接住、模型看到」）⇒
     ⑤ 与硬闸**行为完全不变**，仍然拦闸 / 立刻关闸 ⇒ 这个最重要的兜底没有被削弱。
  2. **已融合**的前车仍由 ④ 两道管住：硬闸 `dRel ≤ 20 m` 立刻关闸、软条件 `dRel ≤ 50 m` 不开闸。
     ⇒ 20 m 内的融合前车照旧**当帧关闸**。
  3. 其余硬闸（`e2e_should_stop` / `gap < 0` / `a_e2e < −2.0` / `plan_a0 < −2.0`）**一律未动**。
  4. 本次**未动** `E2E_GATE_MIN_GAP_MS`（欠速前提保留）、**未动**加速表。
     与同日被实车否决并回退的那一版（同时改「不再要求欠速 + ⑤ + 表值 1.6」）**明确区分**：
     那版三处同批 ⇒ 事后无法归因；**本版单变量**。

预期天花板（**别误解为「能猛加速」**）：
  让位后 `aTgt = min(aMpc, aCruise) = aCruise ≈ 0.37~0.48`（cruise 表 50 km/h 处 0.40）
  ⇒ **只够「不掉速」（> 阻力约 0.3），不够「积极追上设定速度」**。
  后者需要动加速表 —— 那是**第二个变量**，必须另跑一趟实车再叠加。

回退：
  · 一键：`E2E_ACCEL_GATE_ENABLE = False`
  · 只回退本修订：删掉两处 `and not lead_present`（软条件 ⑤ 与硬闸 `hardProb`）
"""
...
def should_yield(
  *,
  ...
  if lead_present and d_rel <= _lead_m:
    return False
  # ⑤ 模型自己看到前车、**且雷达没有融合到它** -> 不开闸
  #    [2026-09-24 第 4 次修订] 追加 `and not lead_present`，理由见文件头「第 4 次修订」：
  #    已被融合的目标由 ④ 按距离判一次就够了；⑤ 与 ④ 判的是**同一辆车**，
  #    而 ⑤ 的门槛（prob 0.5）远比 ④ 的门槛（dRel 50 m）松 ⇒ 62~104 m 的远车
  #    被 ⑤ 卡死，是「该加速却不加速」的主因（实测占 90% 压制帧）。
  if model_lead_prob >= _lead_prob and not lead_present:
    return False
  # ⑥ 模型轨迹在减速 -> 尊重它
  ...
  def update(
    ...
    *,
    ...
    if lead_present and d_rel <= E2E_GATE_HARD_LEAD_M:
      ...
      return False
    # [2026-09-24 第 4 次修订] 同 ⑤ 口径：已被雷达融合的目标交给 ④（含其 20 m 硬闸）
    # 按距离判；只有「模型高置信度看到、而雷达**没**融合到」的前车才立刻关闸。
    # 两条（软 0.5 / 硬 0.8）必须同口径，否则会出现「软条件放行、硬闸立刻关」的自相矛盾。
    if model_lead_prob >= E2E_GATE_HARD_LEAD_PROB and not lead_present:
